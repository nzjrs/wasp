...
#
# converts python list of (x,y,z) readings to a scipy array for furthur analysis
#
def read_list(l):
    assert type(l) in (list, tuple)
    assert len(l[0]) == 3
    assert type(l[0][0]) == float
...
#
def read_log(filename, sensor):
    f = open(filename, 'r')
    pattern = re.compile("IMU_"+sensor+"_RAW (\S+) (\S+) (\S+)")
    list_meas = []
    while 1:
...
def filter_meas(meas, window_size, noise_threshold):
    ...
    for i in range(window_size,len(meas)-window_size):
        ...
        if  scipy.linalg.norm(noise) < noise_threshold:
...
def scale_measurements(meas, p):
    ...
    for m in meas[:,]:
        sm = (m - p[0:3])*p[3:6]
        l_comp.append(sm)
        l_norm.append(scipy.linalg.norm(sm))
